...
test("constructor should time-order its Positions", () => {
  expect(
    basicTrajectory.positions[0].time.equals(time0AsJulianDate),
  ).toBeTruthy();
  expect(
    basicTrajectory.positions[1].time.equals(time1AsJulianDate),
  ).toBeTruthy();
  expect(
    basicTrajectory.positions[2].time.equals(time2AsJulianDate),
  ).toBeTruthy();
});
...
describe("latestPositionWithinWindow()", () => {
  test("should return undefined when there's no Position within the window", () => {
    const position = basicTrajectory.latestPositionWithinWindow(
      JulianDate.fromIso8601("2026-01-01T00:03:01Z"),
      60,
    ); // window starts after time2
    expect(position).toBeUndefined();
  });

  test("should return the latest Position when there are multiple within the window", () => {
    const position = basicTrajectory.latestPositionWithinWindow(
      JulianDate.fromIso8601("2026-01-01T00:01:11Z"),
      20,
    ); // window includes time0 and time1
    expect(position.time.equals(time1AsJulianDate)).toBeTruthy();
  });

  test("should return a Position from the window's end time", () => {
    const position = basicTrajectory.latestPositionWithinWindow(
      JulianDate.fromIso8601(time0),
      10,
    ); // window ends at time0
    expect(position.time.equals(time0AsJulianDate)).toBeTruthy();
  });

  test("should return a Position from the window's start time", () => {
    const position = basicTrajectory.latestPositionWithinWindow(
      JulianDate.fromIso8601("2026-01-01T00:01:51Z"),
      20,
    ); // window starts at time2
