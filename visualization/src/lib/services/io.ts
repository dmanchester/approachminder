 ...
 */
export function constructTrajectoryCollection(
  template: TrajectoryCollectionTemplate,
) {
  const trajectories = template.map((trajectoryTemplatePart) => {
    const positionTemplates: Array<[string, PositionTemplate]> = Object.entries(
      trajectoryTemplatePart.positions,
    ).map(([time, positionTemplatePart]) => {
      const approachSegmentTemplatePart = positionTemplatePart.approachSegment;
      const approachSegment = approachSegmentTemplatePart
        ? new ApproachSegment(
            approachSegmentTemplatePart.airport,
            approachSegmentTemplatePart.threshold,
            approachSegmentTemplatePart.thresholdDistanceMeters,
            approachSegmentTemplatePart.verticalDevMeters,
            approachSegmentTemplatePart.horizontalDevMeters,
            approachSegmentTemplatePart.normalizedEuclideanDistance,
          )
        : null;
    ...
    return new Trajectory(
      trajectoryTemplatePart.icao24,
      trajectoryTemplatePart.callsign,
      trajectoryTemplatePart.category,
