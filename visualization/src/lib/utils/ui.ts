...
export function configureViewer(
  viewer: Viewer,
...
export function createCesiumEntity(
  trajectory: Trajectory,
  airplaneIonResource: IonResource,
): Entity {
  const times = trajectory.positions.map((position) => position.time);
  const positions = trajectory.positions.map((position) =>
