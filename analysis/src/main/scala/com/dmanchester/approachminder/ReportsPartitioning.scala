...
import com.dmanchester.approachminder.PartitioningStateUtils.beginNewAccumulatingPartition
 ...
 */
trait PartitioningState[R <: HasPositionReportIdentifiers] {
  def processReport(report: R): PartitioningState[R]
  def partitions: Seq[(Option[String], Seq[R])]
}

class StateInitial[R <: HasPositionReportIdentifiers](timeGapSecs: Int) extends PartitioningState[R] {
...
class StateAccumulatingWithoutCallsign[R <: HasPositionReportIdentifiers](partitionInProgress: Seq[R], completedPartitions: Seq[(Option[String], Seq[R])], timeGapSecs: Int) extends PartitioningState[R] {
...
class StateAccumulatingWithCallsign[R <: HasPositionReportIdentifiers](partitionInProgress: Seq[R], callsign: String, completedPartitions: Seq[(Option[String], Seq[R])], timeGapSecs: Int /*TODO Is Int big enough?*/) extends PartitioningState[R] {
...
object PartitioningStateUtils {

  def beginNewAccumulatingPartition[R <: HasPositionReportIdentifiers](report: R, completedPartitions: Seq[(Option[String], Seq[R])], timeGapSecs: Int): PartitioningState[R] = {
    report.callsign.map { callsign =>
  ...
  def initial[R <: HasPositionReportIdentifiers](timeGapSecs: Int): PartitioningState[R] = new StateInitial(timeGapSecs)
}
...
object ReportsPartitioning {
  ...
  def partition[R <: HasPositionReportIdentifiers](reports: Iterable[R], timeGapSecs: Int): Seq[(Option[String], Seq[R])] = {
    reports.foldLeft(PartitioningStateUtils.initial[R](timeGapSecs)) { (partitionerState, report) =>
